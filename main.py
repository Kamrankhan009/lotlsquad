...
GOOGLE_CLIENT_ID = "272709683253-mpf5favbofrf4fm1o7smut87gj7fcts1.apps.googleusercontent.com"
# #GOOGLE_CLIENT_SECRET = os.environ.get("GOOGLE_CLIENT_SECRET", None)
GOOGLE_CLIENT_SECRET = "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"
GOOGLE_DISCOVERY_URL = (
    "https://accounts.google.com/.well-known/openid-configuration"
)


# GOOGLE_CLIENT_ID = "272709683253-o1og0c7hhgiu1q6vnaatkpi38ishkbe7.apps.googleusercontent.com"
# GOOGLE_CLIENT_SECRET = "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"
...
def callback():
    ...
    try:
        ...
        red_url = redirect_url.find("/login")

        login_index = auth_url.find("/login")

        if red_url != -1:
            redirect_url = "https://lotlsquad.com" + redirect_url[red_url:]
        
        else:
            pass

        if login_index != -1:
        # Replace the URL before "/login" with "https://lotlsquad.com"
            auth_url = "https://lotlsquad.com" + auth_url[login_index:]
        else:
            print("'/login' not found in the URL.")
    ...
    # by Google
    user = User(
        google_id=unique_id, name=users_name, email=users_email, password="None"
    )

    # Doesn't exist? Add to database
    check_user = User.query.filter_by(google_id=unique_id).first()
    if not check_user:
        db.session.add(user)
        db.session.commit()
    else:
...
def history():
    ...
    order = db.session.query(Orders, OrderItems, Products).join(OrderItems, Orders.id == OrderItems.order_id).join(
        Products, OrderItems.product_id == Products.id).order_by(desc(Orders.id)).paginate(page=page, per_page= row_per_page)
